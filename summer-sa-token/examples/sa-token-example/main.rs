...
async fn main() {
    App::new()
        .use_config_file(concat!(
            ...
            "/examples/sa-token-example/config/app.toml"
        ))
...
async fn get_config(Component(state): Component<SaTokenState>) -> impl IntoResponse {
    ...
    Json(serde_json::json!({
        ...
        "overflow_logout_mode": format!("{:?}", config.overflow_logout_mode),
        "replaced_login_exit_mode": format!("{:?}", config.replaced_login_exit_mode),
        "replaced_range": format!("{:?}", config.replaced_range),
        ...
        "logout_range": format!("{:?}", config.logout_range),
