...
- **changed**: bump `sa-token-plugin-axum` / `sa-token-core` / `sa-token-adapter` / `sa-token-storage-memory` from `0.1.15` to `0.1.18`.
- **changed**: synchronize `SaTokenConfig` with the current sa-token configuration surface and pass every supported field explicitly into `CoreConfig`.
- **breaking**: remove `token_prefix` after sa-token-core dropped it, and remove the plugin-local `storage_prefix` / `rewrite_storage_prefix` options. Use `storage_key_prefix` for storage key namespacing.
- **changed**: `with-summer-redis` storage now uses the keys and patterns generated by sa-token directly, instead of applying an additional plugin-local prefix layer.
- **fixed**: update the example app and tests for the new config fields and Redis storage behavior.
